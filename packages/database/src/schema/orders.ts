import { pgTable, uuid, varchar, timestamp, text, date, numeric, index } from 'drizzle-orm/pg-core';
...
export const orders = pgTable(
  ...
  (table) => ({
    ...
    companyIdIdx: index('idx_orders_company_id').on(table.companyId),
    addressIdIdx: index('idx_orders_address_id').on(table.addressId),
    partnerIdIdx: index('idx_orders_partner_id').on(table.partnerId),
  }),
);
