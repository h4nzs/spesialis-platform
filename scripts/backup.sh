...
#
# Schedule via cron:
#   0 3 * * * cd /opt/ahlipanggilan && bash scripts/backup.sh /mnt/backups
#
...
FILENAME="ahlipanggilan_db_${TIMESTAMP}.sql.gz"
RETENTION_DAYS=30
...
find "$OUTPUT_DIR" -name 'ahlipanggilan_db_*.sql.gz' -mtime "+${RETENTION_DAYS}" -delete
