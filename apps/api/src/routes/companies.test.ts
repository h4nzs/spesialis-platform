import { describe, it, expect, vi, beforeEach } from 'vitest';
import { Hono } from 'hono';
import type { Context } from 'hono';
import type { UserRole } from '@specialist/types';
import { companiesRouter } from './companies.ts';
import { errorHandler } from '../middleware/error-handler.ts';
import { setTestEnv, makeChain, insertChain, updateChain } from '../test-utils.ts';

const { mockDb, authState, mockAuth, em } = vi.hoisted(() => {
  const db = {
    select: vi.fn(),
    insert: vi.fn(),
    update: vi.fn(),
    delete: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(undefined) }),
    execute: vi.fn().mockResolvedValue([]),
    transaction: vi.fn((fn: (tx: unknown) => unknown) => fn(db)),
  };
  const st: { userId: string; userRole: UserRole } = { userId: 'uid', userRole: 'corporate' };
  const mA = {
    hashPassword: vi.fn().mockResolvedValue('h'),
    verifyPassword: vi.fn().mockResolvedValue(true),
    signAccessToken: vi.fn().mockResolvedValue('t'),
    generateRefreshToken: vi.fn().mockReturnValue('r'),
    hashToken: vi.fn().mockReturnValue('h'),
    getRefreshTokenExpiry: vi.fn().mockReturnValue(new Date(Date.now() + 86400000)),
  };
  const exps = (globalThis as Record<string, unknown>).__TABLE_EXPORTS as Record<string, unknown>;
  return { mockDb: db, authState: st, mockAuth: mA, em: exps };
});

vi.mock('../lib/db.ts', () => ({ db: mockDb, ...em }));
vi.mock('../lib/auth.ts', () => ({ ...mockAuth }));
vi.mock('../middleware/auth.ts', () => ({
  authMiddleware: async (c: Context, next: () => Promise<void>) => {
    if (!c.req.header('Authorization')) {
      c.status(401);
      return c.json({ success: false, code: 'UNAUTHORIZED', message: 'No token' });
    }
    c.set('userId', authState.userId);
    c.set('userRole', authState.userRole);
    await next();
  },
  requireRole:
    (...roles: string[]) =>
    async (c: Context, next: () => Promise<void>) => {
      if (!roles.includes(authState.userRole)) {
        c.status(403);
        return c.json({ success: false, code: 'FORBIDDEN', message: 'Forbidden' });
      }
      await next();
    },
}));

function a() {
  return { Authorization: 'Bearer x', 'Content-Type': 'application/json' };
}

function mkApp(role: UserRole = 'corporate') {
  authState.userRole = role;
  authState.userId = 'uid';
  const app = new Hono();
  app.onError(errorHandler);
  app.route('/api/v1/companies', companiesRouter);
  return app;
}

beforeEach(() => {
  setTestEnv();
  vi.clearAllMocks();
  mockDb.transaction.mockImplementation((fn: (tx: unknown) => unknown) => fn(mockDb));
  mockDb.select.mockReturnValue(makeChain([]));
  mockDb.insert.mockReturnValue(insertChain([]));
  mockDb.update.mockReturnValue(updateChain([]));
});

describe('POST / (public registration)', () => {
  it('201 created', async () => {
    mockDb.select.mockReturnValue(makeChain([]));
    mockDb.insert.mockReturnValueOnce(insertChain([{ id: 'uid', role: 'corporate' }]));
    mockDb.insert.mockReturnValueOnce(insertChain([{ id: 'co1' }]));
    mockDb.insert.mockReturnValueOnce(insertChain([{ id: 'cu1' }]));
    const res = await mkApp().request('/api/v1/companies', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        companyName: 'PT Maju',
        legalName: 'PT Maju Sejahtera',
        email: 'corp@maju.com',
        phone: '08123456789',
        password: 'Str0ng!P1',
      }),
    });
    expect(res.status).toBe(201);
  });
  it('409 duplicate email', async () => {
    mockDb.select.mockReturnValue(makeChain([{ id: 'x' }]));
    const res = await mkApp().request('/api/v1/companies', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        companyName: 'PT Maju',
        legalName: 'PT Maju Sejahtera',
        email: 'corp@maju.com',
        phone: '08123456789',
        password: 'Str0ng!P1',
      }),
    });
    expect(res.status).toBe(409);
  });
  it('422 validation', async () => {
    const res = await mkApp().request('/api/v1/companies', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    });
    expect(res.status).toBe(422);
  });
});

describe('GET /', () => {
  it('200 admin', async () => {
    mockDb.select.mockReturnValue(makeChain([{ id: 'co1' }]));
    const res = await mkApp('admin').request('/api/v1/companies', { headers: a() });
    expect(res.status).toBe(200);
  });
  it('200 corporate', async () => {
    mockDb.select.mockReturnValue(makeChain([{ id: 'co1' }]));
    const res = await mkApp('corporate').request('/api/v1/companies', { headers: a() });
    expect(res.status).toBe(200);
  });
  it('403 customer', async () => {
    const res = await mkApp('customer').request('/api/v1/companies', { headers: a() });
    expect(res.status).toBe(403);
  });
});

describe('GET /me', () => {
  it('200', async () => {
    mockDb.select.mockReturnValue(makeChain([{ id: 'co1' }]));
    const res = await mkApp().request('/api/v1/companies/me', { headers: a() });
    expect(res.status).toBe(200);
  });
  it('404', async () => {
    mockDb.select.mockReturnValue(makeChain([]));
    const res = await mkApp().request('/api/v1/companies/me', { headers: a() });
    expect(res.status).toBe(404);
  });
});

describe('PATCH /:id', () => {
  it('200 admin', async () => {
    mockDb.select.mockReturnValue(makeChain([{ id: 'co1' }]));
    const res = await mkApp('admin').request('/api/v1/companies/co1', {
      method: 'PATCH',
      headers: a(),
      body: JSON.stringify({ companyName: 'PT Maju Jaya' }),
    });
    expect(res.status).toBe(200);
  });
});

describe('POST /:id/verify', () => {
  it('200 verified', async () => {
    mockDb.select.mockReturnValue(makeChain([{ id: 'co1', status: 'Pending' }]));
    const res = await mkApp('admin').request('/api/v1/companies/co1/verify', {
      method: 'POST',
      headers: a(),
      body: JSON.stringify({ status: 'Verified' }),
    });
    expect(res.status).toBe(200);
  });
  it('403 customer', async () => {
    const res = await mkApp('customer').request('/api/v1/companies/co1/verify', {
      method: 'POST',
      headers: a(),
      body: JSON.stringify({ status: 'Verified' }),
    });
    expect(res.status).toBe(403);
  });
});
